 ...
 */
int ecx_setupnic(ecx_portt *port, const char *ifname, int secondary, int CS)
{
   int i;
   int *psock;
   ...
   // open (=> SOEM.cpp)
   int result = hal_ethernet_open(CS);
...
int ec_setupnic(const char *ifname, int secondary, int CS)
{
   return ecx_setupnic(&ecx_port, ifname, secondary, CS);
}
