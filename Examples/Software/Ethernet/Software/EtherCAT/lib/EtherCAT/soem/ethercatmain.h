...
#define EC_MAXSLAVE       64 // TODO shrinked for memory reduction (original = 200)
...
struct ecx_context
{
   ...
   /** registered EoE hook */
   int            (*EOEhook)(struct ecx_context * context, uint16 slave, void * eoembx);
};
...
int ec_init(const char * ifname, int CS);
// secondary port is not supported
// because GR-SAKURA/ROSE has only one LAN port
#if 0
...
int ecx_init(ecx_contextt *context, const char * ifname, int CS);
// secondary port is not supported
// because GR-SAKURA/ROSE has only one LAN port
#if 0
