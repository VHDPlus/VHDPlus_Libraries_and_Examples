...
#else
	select();
	transfer(SPIFLASH_IDREAD);
#endif
	uint16_t jedecid = transfer(0) << 8;
	jedecid |= transfer(0);
	unselect();
	return jedecid;
}
...
void SPIFlash::readBytes(uint32_t addr, void* buf, uint16_t len) {
	...
	transfer(addr >> 8);
	transfer(addr);
	transfer(0); //"dont care"
	for (uint16_t i = 0; i < len; ++i)
		((uint8_t*)buf)[i] = transfer(0);
	unselect();
}
